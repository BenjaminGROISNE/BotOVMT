...
bool fadb::onScreen(std::string package) {

    std::string command = "dumpsys activity | findstr mResumedActivity | findstr " + package;
    std::string result = runAdbLocalhostShell(command);
    if (result.find(package) != std::string::npos) {
        return true;
    }
    else {
        std::cout << "Package is not in the foreground." << std::endl;
        return false;
    }
}
...
void fadb::takeScreenshot(const std::string& filename) {
    std::string command = "screencap /sdcard/" + filename;
    std::string commandPull = "pull /sdcard/" + filename + " ./img";
    runAdbLocalhostShell(command);
    runAdbLocalhost(commandPull);
}
...
bool fadb::onApp(std::string package) {
    std::string output = runAdbLocalhostShell("dumpsys window windows | grep -E 'mFocusedApp='");
    if (output.find(package) != std::string::npos) {
        std::cout << "On " + package << std::endl;
        return true;
    }
    else {
        std::cout << "not on " + package << std::endl;
        return false;
    }
}
