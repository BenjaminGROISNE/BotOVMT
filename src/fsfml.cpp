...
void fsfml::initWindow(){
	window=new sf::RenderWindow(sf::VideoMode(screenWidth/1.5, screenHeight/1.5), "Window", sf::Style::Default);
...
void fsfml::windowLoop()
{
	...
	while (window->isOpen())
	{
		...
		if (std::tolower(charEntered) == 'b') {
			std::cout << "Start Collecting text" << std::endl;
			collectText = true;
		}
		else if (std::tolower(charEntered) == 'e') {
			std::cout << "Stop collecting text" << std::endl;
			collectText = false;
		}
...
void fsfml::eventLoop()
{
	...
	while (window->pollEvent(event))
	{
		...
		case sf::Event::MouseEntered:
			std::cout << "MouseInWindow\n";
			mouseInWindow = true;
			break;
		case sf::Event::MouseLeft:
			std::cout << "MouseOutsideWindow\n";
			mouseInWindow = false;
			break;
...
void fsfml::initMenu()
{
	std::vector<std::string> allstr = { imageDir, alchemy ,arena ,assistant ,banquet ,heirloom,mail,packs,perks,ranking,worldrift };
	std::string id="Menu";
...
void fsfml::initGraphics()
{
	...
	add(allpaths, alchemy);
	add(allpaths, arena);
	add(allpaths, assistant);
	add(allpaths, banquet);
	add(allpaths, garden);
	add(allpaths, heirloom);
	add(allpaths, mail);
	add(allpaths, packs);
	add(allpaths, perks);
	add(allpaths, ranking);
	add(allpaths, worldrift);
