...
void sysCommands::writeFilesDirectory(std::filesystem::directory_iterator path, std::ofstream& ofs) {
    ...
    std::string FOLDEROUTPUT = "std::string " + folderName + " = " + currentPath.parent_path().filename().string() + " + \"" + folderName + "/" + "\";" + "\n";
    //std::cout << std::endl;
    //std::cout << com;
    //std::cout << FOLDEROUTPUT;
    ofs << "\n" << com << FOLDEROUTPUT;
    ...
    for (const auto& entry : path) {
        if (entry.is_regular_file()) {
            ...
            std::string filename = "std::string " + entry.path().stem().string() + " = " + folderName + " + " + "\"" + entry.path().filename().string() + "\";" + "\n";
            //   std::cout << filename;
            ofs << filename;
        }
    ...
    for (int i = 0; i < folders.size(); i++) {
        if (!std::filesystem::is_empty(folders.at(i).path())) {
            writeFilesDirectory(std::filesystem::directory_iterator(folders.at(i).path()), ofs);
        }
        else {
...
void sysCommands::createpaths() {
    std::ofstream ofs("paths.txt");
    std::string path = ".\\imgObjects";
    std::filesystem::directory_iterator dir(path);
    writeFilesDirectory(dir, ofs);
    ofs.close();
}
